...
| UT-054 | decision | Maya / W07-W08 fresh-assignment gate | `user-testing-output/plane-agents/evidence/w07-w08-fresh-assignment-gate-358de27c.json` SHA-256 `7ee48318fd710596492a16704e384a47b81688d179862e79d70d6dc9f194ea9f` | Reconciled the earlier provider-free exact-request proof with the committed bounded reasonSubreason contract. Host suites passed `180/180`; serial relay/lifecycle/API checks passed `17/17`; provider calls/attempts/replays were `0`. No local request/auth/config defect was proven. | NO_GO now: the prior terminal provider 4xx remains an unresolved external acceptance/credential disposition, and the new bounded field cannot retroactively classify that old receipt. | Permit exactly one fresh assignment only after a current authoritative external acceptance disposition, clean candidate refreeze, and non-vacuous runtime/DB/capacity gates. Never replay the prior invocation. | open |

...
function render() {
  const gateRows = markdownTable(
    ["Gate", "Meaning", "Representative exit condition"],
    plan.gates.map((gate) => [gate.id, gate.name, gate.exit[0]])
  );
  const laneRows = markdownTable(
    ["Lane", "Responsibility", "Accountable owner", "Starts after", "Complete by"],
    plan.lanes.map((lane) => [lane.id, lane.name, lane.owner, lane.startAfter ?? "Now", lane.finishBy])
  );

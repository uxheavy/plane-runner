...
- G0 freezes the logical plane.agent-runtime/v1 boundary, dispatch semantics, event/publication variants, catalog names, limits, retention, idempotency, and audit failure policy; generated operation/event schemas are a G1 input and the physical durable queue/RPC transport remains implementation-defined under ADR-0010.
...
- Freeze the semantic operation catalog, model-facing presentation, logical runtime/event/publication boundary, limits, retention, and failure semantics; leave generated schemas to G1 and physical transport to ADR-0010's later gate.

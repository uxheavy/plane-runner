...
| Live W/M/O wave    | Wave 0BA reached two fresh Maya identity commissions with completed provider attempts, but the invoker mislabeled later local scenario-gate failure as reconciliation uncertainty. The first f63 post-fix identity commission then exposed redundant model `run_ref` poisoning; Plane fix `8681f2e7db` normalizes that payload, preserves trusted envelope binding, orders preflight before terminal readback, and returns truthful terminal conflicts/no-mutation observations. W01/W02 remain dirty pending a fresh primary plus eligible replay; W03–W08 are unreached. The pinned Hermes `bc7f13d2...` still needs its expected `PLANE_CONFLICT` nonfatal disposition before provider use. O02 remains separately clean from its real external-client closure.         |

...
- Every implementation task must use the Arena workflow: Frame the shared artifact and gradeable rubric; Fan out at least two GPT-5.6 Luna High candidates into isolated worktrees or output paths; Cross-judge only after all candidates finish; Pick one base after reading every candidate; Graft only the strongest compatible parts; and Verify the synthesized result. Use Luna xhigh where route complexity requires it. Record one compact synthesis note with the base, grafts, rejections, dropouts, and verification result. Read-only diagnosis and preparation do not require an Arena unless they produce code. Archive Arena tasks after their accepted work is integrated. Sol Medium remains reserved for the consolidated reviews below, not Arena implementation judging.

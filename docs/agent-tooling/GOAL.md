...
| G4 offline baseline   | The canonical wrapper is an exact single child of implementation parent `61a73f8719651871afa6f6a6bcbb856a8618f56f`, with descendant rejection, accepted G3 baseline `7c9d35f4c324865c27c84da5016be2c84e460bcc`, and the full offline proof recorded below. |
| Current HEAD          | The integrated local-topology implementation is bound by the canonical one-child wrapper: one shared runtime settings parser, local Compose extending the canonical community runtime service, a fixed canonical community entrypoint, and proof of both ordinary local startup and the explicit agent profile. |

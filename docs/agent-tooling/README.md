...
G0 through G3 are complete. The previously accepted canonical offline G4 wrapper remains the baseline; the current remediation source correction is `e9010dec4adde127b4a7249b06efdd1356e3628e`, with one exact wrapper child still requiring fresh offline proof before it can replace that baseline. The existing API/runtime image pins remain bound to image-source commit `c47ddfe6174ecd6d66257d8fedbd5d425c7f3172` and are recorded in [GOAL.md](./GOAL.md). The remediation uses the external `PLANE_G4_EXPECTED_CANDIDATE` authority, one shared G3/G4 verifier lock, Docker-visible Hermes preflight, and a retained sanitized receipt. The latest authorized live attempt remains `outcome_unknown` and will not be blindly replayed; this offline repair made no provider request or retry. Live G4 and staged G5 remain incomplete. No chat UI is in scope.

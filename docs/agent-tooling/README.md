...
G0 through G3 are complete. The previously accepted canonical offline G4 wrapper remains the baseline; the current remediation source correction is `9ccba1ef04f78f3cc1e9fcec37b30e38041dc1a8`, with one exact wrapper child still requiring fresh offline proof before it can replace that baseline. The existing API/runtime image pins remain bound to image-source commit `c47ddfe6174ecd6d66257d8fedbd5d425c7f3172` and are recorded in [GOAL.md](./GOAL.md). The remediation uses the external `PLANE_G4_EXPECTED_CANDIDATE` authority, one shared G3/G4 verifier lock, and a retained sanitized receipt. The latest authorized live attempt remains `outcome_unknown` and will not be blindly replayed; this offline repair made no provider request or retry. Live G4 and staged G5 remain incomplete. No chat UI is in scope.

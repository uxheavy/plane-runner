...
| VM-011 | Concurrency and load                     | Ordering, retry races, rate limits, approved concurrency, duration, p95/p99, and recovery targets pass                                                                                                                                           | Release-like stack                              | No                                   | Metrics and traces                             |
| VM-012 | External MCP inventory and compatibility | Every current Python MCP tool has one disposition, every reachable handler SDK call maps through a pinned method/path-to-versioned-operation table, and pinned real clients pass                                                                 | Real MCP clients plus gateway                   | No                                   | Mappings, client versions, transcripts         |
| VM-013 | Mandatory live project planning          | Frozen prompt through real Luna Hermes autonomously produces exact tagged artifacts within its authorized scope and leaks no control-project data                                                                                                | Authenticated Plane dev server plus real Hermes | None                                 | Transcript, traces, UI/API and audit readbacks |
...
| VM-015 | Operator lifecycle                       | Provision, permissions, credential issue/store/rotate/revoke, old-credential denial, audit lookup, kill switches, recovery, and rollback succeed                                                                                                 | Release-like stack                              | No                                   | Exercise logs and readbacks                    |
...
Before this verification manifest can be approved, the version-controlled verifier specification must expand this table to a requirement-level matrix. Every normative bullet in `GOAL.md` and every row in the approved release manifest must map to at least one check, one observable oracle, and one immutable evidence record. The documentation validator fails when any requirement has zero checks.
...
The final verifier must be executed independently from clean Plane, Hermes, official MCP, and Plane Python SDK checkouts at the exact integration-lock commits. It must prove its own sensitivity by passing VM-018 through VM-021 as expected failures before its positive result is accepted.

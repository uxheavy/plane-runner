...
| VM-005 | Autonomous execution matrix              | Authorized calls never pause; denied calls have no effects; no human-confirmation route, approval result state, approval broker credential, approval configuration, or approval persistence model exists; preflight only validates schema, references, authorization, budgets, and concurrency; every supported route crosses application services through the gateway with no direct-database bypass                                                                                                                                                                                                          | Plane plus real Hermes                          | No in final layer                    | Routes, dependency inventory, schemas, events, state, and audit              |
| VM-006 | Audit durability                         | Intent and outcome exist for invalid, denied, failed, unknown, and successful attempts; intent commits before dispatch; mutation, terminal-success audit, and invocation result use one named PostgreSQL connection/transaction; separately injected success-audit insert, result-transition, pre-commit-request, and server-rejection-proven-not-committed failures prove rollback, then append one failed outcome and move the invocation to retryable before response; missing commit acknowledgement returns outcome_unknown and reconciles without blind retry; on-commit tasks never publish on rollback | Plane test stack                                | Failure injection allowed            | Transaction trace, database/API readback, broker capture, and logs           |

...
## Frozen in controlling manifest pending approval

These rows are frozen as a candidate contract by `APPROVAL-MANIFEST.md`. They become accepted only when the user records the exact approval statement in that manifest; until then they do not authorize implementation.

| ID      | Candidate-frozen decision                                                                                                                                                                                                                                                                                                                          | Gate                   |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| ATO-003 | The first supported semantic boundary is the nine existing major-version operation IDs in `RELEASE-MANIFEST.md` and `PILOT-CONTRACTS.md`, with trusted workspace binding and the approved discovery/composition adapters.                                                                                                                          | G0 / manifest approval |
| ATO-004 | The catalog uses the manifest's explicit curated overlay fields for purpose, model description, examples, aliases, schemas, classification, safety, authorization, idempotency, reconciliation, result/artifact policy, audit redaction, composition/retained behavior, adapters, compatibility, disclosure, promotion, retirement, and lifecycle. | G0 / manifest approval |
| ATO-005 | Direct-tool promotion requires a common, stable Plane workflow with a complete typed contract and evidence that eager disclosure improves the assigned workflow without creating a permission tier.                                                                                                                                                | G0 / manifest approval |
| ATO-006 | Direct-tool retirement occurs when the contract is superseded, the operation is no longer supported, or retained eager disclosure is not justified by current workflow evidence; retirement is additive/versioned and cannot silently remove catalog discoverability.                                                                              | G0 / manifest approval |
| ATO-007 | Per-operation result limits use the existing v1 execution/result/artifact table in `APPROVAL-MANIFEST.md`.                                                                                                                                                                                                                                         | G0 / manifest approval |
| ATO-008 | Cumulative source, call, concurrency, preflight, output, stdout/stderr, and wall limits use the existing v1 table in `APPROVAL-MANIFEST.md`.                                                                                                                                                                                                       | G0 / manifest approval |
| ATO-009 | Temporary authoritative artifacts are limited to 10 MiB, retained for 1 hour, and cleaned within a 15-minute lag; bounded reads are limited to the existing 32 KiB / 23,000 decoded-byte contract.                                                                                                                                                 | G0 / manifest approval |
| ATO-016 | Invocation and audit metadata are retained for at least 365 days; bulky full results are never retained by default, while the result digest and bounded redacted summary remain.                                                                                                                                                                   | G0 / manifest approval |
| ATO-012 | Additive compatible catalog changes preserve a major operation ID; incompatible input, output, error, authorization, or idempotency changes require a new major operation ID and a new pinned digest.                                                                                                                                              | G0 / manifest approval |
| ATO-013 | Native adapters use the approved natural names and map to versioned semantic operations; promotion and retirement follow ATO-005/006 without changing global catalog coverage.                                                                                                                                                                     | G0 / manifest approval |
| ATO-019 | The minimal eager direct pilot set is `search_workspace`, `search_work_items`, `get_work_item`, `create_work_item`, `update_work_item`, and `create_comment`; all other semantic operations are progressive or composition-backed.                                                                                                                 | G0 / manifest approval |
| ATO-021 | TypeScript composition is model-facing `compose_typescript`, backed by `plane.typescript.compose@1`, with credential-free host callbacks and the frozen v1 limits; it is not generic `execute`.                                                                                                                                                    | G0 / manifest approval |
| ATP-005 | The manifest promotes the existing v1 execution, result, artifact, audit-retention, and transaction/audit-failure values from the release and pilot evidence inputs.                                                                                                                                                                               | G0 / manifest approval |

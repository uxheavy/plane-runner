...
## 2026-07-30 — Executable safety contract core added

### Candidate artifacts

- Added exact EV-011 through EV-030 autonomous prompts and exact EV-021 through EV-028 TypeScript probe programs.
- Added strict evidence-record, trial-result, qualification-result, and safety-fixture Draft 2020-12 schemas.
- Added a trusted-parent parser profile for all 12 EV-026 import submissions and a separate 17-subcase model-Worker profile; the combined trial remains exactly 29 hostile submissions and 58 callback controls.
- Added a flat content-addressed bundle schema and a qualification-lock schema pinning repositories, builds, configuration, manifests, catalog, runtime, provider `openai-codex`, model `gpt-5.6-luna`, and separated evidence producers.
- Added exact candidate predicate, operator-registry, mutation-class, and predicate-instance applicability contracts. Candidate state contains no fake module digests or mutation cases; qualifying state requires pinned implementations and complete control coverage.

### False-pass corrections

- Made `qualified` structurally require satisfied predicates, detected applicable controls, zero failure counts, and verified semantic-ID, reference, and trial/run/nonce integrity.
- Bound every prompt and applicable probe path, every EV-021 through EV-028 program ABI, and exact ordered EV-023 through EV-026 subcases.
- Required independent Plane readback and closed role-to-producer/channel partitions; removed the nonexistent outbox trace and required broker-publication plus activity evidence for EV-029 and EV-030.
- Made evidence authenticity a mandatory non-circular precondition over the complete acquired evidence index and receipts before predicate evaluation.
- Separated the bundle-owned `producer_false_pass` control from scenario predicates and gave it an explicit control-qualifier target.
- Defined candidate/qualifying mutation states, exact 12 canonical mutation classes/cases, a qualifying 40-predicate-by-12-case applicability product, and one independent producer-false-pass control cell.
- Prohibited bundle-manifest self-inclusion, qualification-lock cross-pin cycles, and credential-bearing, query-bearing, or fragment-bearing HTTPS pins.

### Validation evidence

- Strict AJV 8.18 Draft 2020-12 compilation passed for every added schema and every current candidate instance.
- Exact cross-file joins passed for 40 predicates, 20 operators, 20 scenario bindings, one mandatory acquisition precondition, and 12 mutation classes.
- Prompt/probe ABI, EV-026 parser/runtime arithmetic, formatter, OxLint, staged-diff, and Gitleaks checks passed.
- Targeted negative controls rejected wrong qualification outcomes, prompt/probe substitutions, duplicate or reordered probes, producer/channel swaps, invalid program fixtures, wrong provider/model pins, URL credentials, manifest cycles, operator tuple swaps, mutation-class drift, and weakened companion rules.
- Independent red-team passes closed every reported P0, P1, and P2 in the prompt/probe/schema and bundle/lock slices before their commits.
- Independent red-team passes also closed every reported P0, P1, and P2 in the predicate, operator-registry, mutation, and applicability contracts, including authoritative evidence anti-vacuity, exact two-sided controls, and detector ownership.
- Committed the core prompt/probe/schema slice as `242c4b73ec` and the bundle/qualification-lock schema slice as `5e37697386`.

### Qualification state

These are executable candidate contracts, not product implementation, manifest approval, qualifying evidence, or production readiness. Deno is not installed locally, so the probe programs have formatter/parser/lint and structural validation but not pinned-runtime execution evidence yet.

### Next action

Finish and independently review the safety fixture instance, pinned operator/mutation modules, applicability matrix, three verifier entrypoints, negative controls, and qualification lock instance. Then freeze and obtain the required release- and verification-manifest approvals before product implementation begins.

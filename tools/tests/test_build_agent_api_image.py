...
def test_tampered_runtime_contract_is_rejected(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    api_directory = tmp_path / "api"
    api_directory.mkdir()
    for source in builder.RUNTIME_CONTRACT_DIRECTORY.glob("*.json"):
        (api_directory / source.name).write_bytes(source.read_bytes())
    manifest = api_directory / "manifest.json"
    ...
    monkeypatch.setattr(builder, "RUNTIME_CONTRACT_DIRECTORY", api_directory)

...
| Delivery repository  | [`uxheavy/plane-runner`](https://github.com/uxheavy/plane-runner)                                 |
| Integration branch   | `preview`                                                                                         |
| Top-down thread role | Product design, UX, system architecture, engineering boundaries, and milestone review; no coding  |
...
## `ux/plane-runner` Delivery Ledger

| ID  | State                  | Evidence                                                                                                                                                                | Product meaning                                                                                                                                       | Remaining boundary                                                                                                                                       |
| --- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | Merged                 | [PR #1: semantic context picker core](https://github.com/uxheavy/plane-runner/pull/1) merged into `preview` on 2026-07-29. PR checks and final automated review passed. | Plane now has a reusable non-UI core for typed entity, field, editor-block, editor-range, region, and visual context selection.                       | This is an engineering foundation, not a user-visible preview experience.                                                                                |
| R2  | Merged                 | `packages/chat-context` plus `POST /api/workspaces/{workspaceSlug}/chat-context/hydrate/`.                                                                              | Browser observations retain Plane identity; server hydration rechecks workspace, project, guest, private-object, and deletion rules before agent use. | The chat composer and attachment persistence path are not connected.                                                                                     |
| R3  | Merged                 | Client-store and live-editor observations remain separate from server-canonical values; visual capture requires an explicit preview and stays in memory.                | The implementation preserves freshness, authorization, and privacy boundaries instead of treating visible DOM content as trusted context.             | Activation, crosshair, hover feedback, chips, previews, removal, and send behavior belong to the composer UI work.                                       |
| R4  | Infrastructure blocked | Post-merge `Branch Build CE` runs failed while logging into Docker Hub because repository credentials were absent; CodeQL passed.                                       | The failures do not establish a product-code regression.                                                                                              | No deployable branch-preview image has been verified. Configure repository-owned image publishing or replace the inherited upstream publishing workflow. |
| R5  | Operational risk       | The `preview` branch is currently unprotected.                                                                                                                          | The integration branch can accept changes without enforced review or required checks.                                                                 | Define the minimum branch protection and merge gate before parallel delivery expands.                                                                    |
